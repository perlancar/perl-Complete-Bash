...
subtest "variable substitution" => sub {
    ...
    is_deeply(parse_cmdline(_l(q|a^ $var|)), [['a', 'foo'], 0]);
    # not performed on current word
    is_deeply(parse_cmdline(_l(q|a $var^|)), [['a', '$var'], 1]);
    is_deeply(parse_cmdline(_l(q|a $var^ $var$var|)), [['a', '$var', 'foofoo'], 1]);
    is_deeply(parse_cmdline(_l(q|a^ $var2|)), [['a', ''], 0]);

    # escape prevents variable substitution
    is_deeply(parse_cmdline(_l(q|a^ \\$var|)), [['a', '$var'], 0]);
